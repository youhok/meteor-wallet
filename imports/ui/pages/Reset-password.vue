...
import { useRouter } from "vue-router"; // Import useParams
import { Accounts } from "meteor/accounts-base";
const router = useRouter();


let form = reactive({ password: "" });
...
const resetPassword = () => {
  ...
  if (form.password === "") {
    push.error("Please provide your new password.");
    return;
  }

  // Use the retrieved token to reset the password
  Accounts.resetPassword(token, form.password, (error) => {
    if (error) {
      push.error("Error trying to reset the password");
      console.log(error.message);
    } else {
      form.password = ""; // Clear the password field after successful submission
      push.success("Your new password is set, please sign in!");
