  ...
  <div class="flex flex-col h-screen bg-gray-100">
    ...
    <div class="grid place-items-center mx-2 my-20 sm:my-auto">
      ...
      >
        ...
        <form class="mt-10" @submit.prevent="forgetPassword">
          <!-- Email Input -->
          <label
            for="email"
          ...
          <div class="sm:flex sm:flex-wrap mt-8 sm:mb-4 text-sm text-center">
            <router-link class="flex-2 underline" :to="{ name: 'sign-in' }">
              Back to Sign In 
            </router-link>
          </div>
        </form>
...
const forgetPassword = () => {
  if (form.email === "") {
    push.error("Please provide your email address.");
    return;
  }

  Accounts.forgotPassword({ email: form.email }, (error, result) => {
    if (error) {
      push.error("Error requesting the link to reset your password.");
      console.log(error.message);
    } else {
      form.email = "";
      push.success("You should receive a reset email shortly!");

      if (result && result.token) {
        router.push({ name: "reset-password", params: { token: result.token } });
      } else {
        push.error("Token not found in response.");
        console.log("Reset password token not found in response:", result);
      }
    } 
  });
};
